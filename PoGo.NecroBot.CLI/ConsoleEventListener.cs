...
namespace PoGo.NecroBot.CLI
{
    ...
    internal class ConsoleEventListener
    {
        ...
        {
            var name = ConvertPokemonName.ToJapanese(Convert.ToString(pokemonEvolveEvent.Id));

            Logger.Write(pokemonEvolveEvent.Result == EvolvePokemonResponse.Types.Result.Success
                ? session.Translation.GetTranslation(TranslationString.EventPokemonEvolvedSuccess, name, pokemonEvolveEvent.Exp)
            ...
            if (pokemonEvolveEvent.Result == EvolvePokemonResponse.Types.Result.Success)
			{
				AddToDataView.AddDataEvolve(name, pokemonEvolveEvent.Exp);
			}
        }

        private static void HandleEvent(TransferPokemonEvent transferPokemonEvent, ISession session)
        {
			var name = ConvertPokemonName.ToJapanese(Convert.ToString(transferPokemonEvent.Id));
            ...
            AddToDataView.AddDataTransfer(name, transferPokemonEvent.Cp, transferPokemonEvent.BestCp, transferPokemonEvent.Perfection, transferPokemonEvent.BestPerfection, transferPokemonEvent.FamilyCandies);
        }

        private static void HandleEvent(ItemRecycledEvent itemRecycledEvent, ISession session)
        {
			var item = ConvertItemName.ToJapanese(Convert.ToString(itemRecycledEvent.Id));

            Logger.Write(session.Translation.GetTranslation(TranslationString.EventItemRecycled, itemRecycledEvent.Count, item),
                LogLevel.Recycling);
        }
        ...
        {
			var name = ConvertPokemonName.ToJapanese(Convert.ToString(eggHatchedEvent.PokemonId));

            Logger.Write(session.Translation.GetTranslation(TranslationString.IncubatorEggHatched,
                name, eggHatchedEvent.Level, eggHatchedEvent.Cp, eggHatchedEvent.MaxCp, eggHatchedEvent.Perfection),
                LogLevel.Egg);
        }
        ...
        private static void HandleEvent(FortUsedEvent fortUsedEvent, ISession session)
        {
        	...
        	var item = ConvertItemName.ItemStringToJapanese(itemString);
            ...
            Logger.Write(
                session.Translation.GetTranslation(TranslationString.EventFortUsed, fortUsedEvent.Name, fortUsedEvent.Exp, fortUsedEvent.Gems,
                    item, fortUsedEvent.Latitude, fortUsedEvent.Longitude),
                LogLevel.Pokestop);

            AddToDataView.AddDataPokeStop(fortUsedEvent.Name, item, fortUsedEvent.Gems, fortUsedEvent.Exp);
        }
        ...
        private static void HandleEvent(PokemonCaptureEvent pokemonCaptureEvent, ISession session)
        {
        	...
        	var name = ConvertPokemonName.ToJapanese(Convert.ToString(pokemonCaptureEvent.Id));

            string message;

            if (pokemonCaptureEvent.Status == CatchPokemonResponse.Types.CatchStatus.CatchSuccess)
            {
               message = session.Translation.GetTranslation(TranslationString.EventPokemonCaptureSuccess, catchStatus, catchType, name,
			   ...
			   AddToDataView.AddDataPokemon(name, pokemonCaptureEvent.Level, pokemonCaptureEvent.Cp, pokemonCaptureEvent.MaxCp, Math.Round(pokemonCaptureEvent.Perfection), pokemonCaptureEvent.Exp, pokemonCaptureEvent.FamilyCandies);
			}
            else
            {
                message = session.Translation.GetTranslation(TranslationString.EventPokemonCaptureFailed, catchStatus, catchType, name,
                pokemonCaptureEvent.Level, pokemonCaptureEvent.Cp, pokemonCaptureEvent.MaxCp, pokemonCaptureEvent.Perfection.ToString("0.00"), pokemonCaptureEvent.Probability,
                pokemonCaptureEvent.Distance.ToString("F2"),
                returnRealBallName(pokemonCaptureEvent.Pokeball), pokemonCaptureEvent.BallAmount,
        ...
        {
        	var name = ConvertPokemonName.ToJapanese(Convert.ToString(noPokeballEvent.Id));

            Logger.Write(session.Translation.GetTranslation(TranslationString.EventNoPokeballs, name, noPokeballEvent.Cp),
                LogLevel.Caught);
        }
